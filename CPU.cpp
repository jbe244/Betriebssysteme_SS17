...
bool CPU::write(int adress)
{
	...
	RAM->bytes[mmu->translate(adress)] = aktiver_Prozess->get_writeb();
        //aktiver_Prozess->get_Seitentabelle()->seiten.at(mmu->translate(adress))->mod = true;   //Modify Bit der aktuell veränderten Seite setzen --> crasht Programm
...
int CPU::Adresse_generieren_delta() 
{
    ...
    int value = aktiver_Prozess->get_size() * Seitengroesse;
    ...
    if(random < 50)
    {
        ...
        {
            old_adress = aktiver_Prozess->used_adr.back();
            aktiver_Prozess->used_adr.push_back(old_adress + Delta);
            return old_adress + Delta;
        }
        else
